...
if 'last_note_correct' not in st.session_state:
    st.session_state.last_note_correct = False
...
def check_key_press(note):
    if note == st.session_state.current_key:
        ...
        play_note_if_correct(note)  # Play the note only if correct
        st.session_state.last_note_correct = True
    else:
...
try:
    st.image(KEY_SCORES[st.session_state.current_key], use_column_width=False)
except Exception as e:
    st.error(f"Error loading image: {e}")
    st.markdown(f"<div style='font-size:24px; color:red;'>Error displaying {st.session_state.current_key}</div>", unsafe_allow_html=True)

columns = st.columns(len(NOTE_FILES))

for note, (style, label) in zip(NOTE_FILES.keys(), generate_keys_layout(range(6, 7))):
    with columns[list(NOTE_FILES.keys()).index(note)]:
...
if st.button("Refresh Note Score"):
    if st.session_state.last_note_correct:
        st.session_state.current_key = random.choice(list(KEY_SCORES.keys()))
        st.session_state.feedback_message = ""
        st.session_state.last_note_correct = False
